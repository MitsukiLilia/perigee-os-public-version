...
**EN:** Perigee OS is a front-end tool only. It does **not** provide any AI service, model, or API key — users connect their own third-party API credentials. All content generated through this software is produced by the user's chosen AI provider at the user's direction, and the user is **solely responsible** for that content and for complying with all applicable laws and the terms of their AI provider. The authors provide this software "AS IS" under the PolyForm Noncommercial License 1.0.0 (noncommercial use only) and accept **no liability** for any use, content, or consequence arising from it.

**中文：** Perigee OS 仅为前端工具，**不提供**任何 AI 服务、模型或 API 密钥，用户需自行接入第三方 API。经本软件生成的一切内容，均由用户自行选择的 AI 服务商按用户指令产生，内容及其合法性、是否遵守所在地法律与 AI 服务商条款，**由用户自行承担全部责任**。本软件依 PolyForm 非商业许可证 1.0.0（PolyForm Noncommercial License 1.0.0，仅限非商业用途）以"现状"提供，作者对任何使用、内容或由此产生的后果**不承担任何责任**。
...
[PolyForm Noncommercial 1.0.0](./LICENSE) © 2026 MitsukiLilia

This project is licensed under the **PolyForm Noncommercial License 1.0.0**. You may use, modify, distribute, and self-host it **for noncommercial purposes only** — personal use, hobby projects, research, education, and noncommercial organizations are all permitted. **Commercial use is not permitted.** The software is provided "as is", without any warranty, and the author accepts **no liability**.

本项目采用 **PolyForm 非商业许可证 1.0.0（PolyForm Noncommercial License 1.0.0）**。你可以**出于非商业目的**自由使用、修改、分发、自部署 —— 个人使用、兴趣项目、研究、教育及非商业组织均在允许之列。**禁止任何商业用途。** 本软件按「现状」提供，不含任何担保，作者**不承担任何责任**。
