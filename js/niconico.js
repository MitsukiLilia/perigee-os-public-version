    ...
    renderVideoDetail() {
        ...
        <div class="nico-detail-info">
            ...
            <div class="nico-detail-actions">
                ...
                ${(v.lyricCues && v.lyricCues.length && this._pvLyricToggleBtnHtml) ? this._pvLyricToggleBtnHtml() : ''}
                ${(v.videoBlobId && this._pvSaveBtnHtml) ? this._pvSaveBtnHtml(v.id) : ''}
                ${v.videoBlobId ? `
...
- 🚫 設定にないストーリーを捏造するな`;
    ...
    _stopRealPlayer() {
        // 歌词字幕（六期 P2，niconico-pv-lyrics.js）：先停掉可能在跑的字幕 rAF 循环，再处理 videoEl 本身——
        // 即使当前没有 videoEl（比如已经不在详情页）也要清一次，防止上一个视频的循环悬空继续跑
        if (this._pvLyricDetach) this._pvLyricDetach();
