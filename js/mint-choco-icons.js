// ===== 薄荷巧克力(mint-choco)主题 · 日系平涂波点卡图标 =====
// 仅在 mint-choco 主题下，把桌面 app 图标替换成波点卡 webp；切走恢复默认。
// 用户自定义图标（AppState.data.customIcons）优先级最高，不被覆盖。
// data-icon-default 与 Constellation/Journal/Strawberry/Snow/Sakura/Animal/Rain/TaroChoco 各图标模块共享，
// 九方守卫互相对称（切走任一图标主题时，若目标是另八个图标主题之一则不恢复、交给目标接管）。
// 调用时机：DesktopRenderer.render() 末尾 + SystemConfig.applyTheme() 末尾。

const MintChocoIcons = {
  THEME: 'mint-choco',

  // app id → webp 文件名（assets/icons/mint-choco/ 目录下，文件名即 appId）
  ICONS: {
  ...
  apply() {
    const isMintChoco = document.documentElement.dataset.theme === this.THEME;
    ...
    document.querySelectorAll('.app-item[data-app]').forEach(item => {
      ...
      if (isMintChoco && this.ICONS[appId]) {
        // 首次替换：把原始 SVG 存起来（与各图标主题模块共用 data-icon-default，只存一次）
        if (!c.dataset.iconDefault) c.dataset.iconDefault = c.innerHTML;
        c.innerHTML = `<img src="assets/icons/mint-choco/${this.ICONS[appId]}" alt="${appId}" draggable="false">`;
      } else if (c.dataset.iconDefault) {
        // 离开薄荷巧克力主题：恢复原始 SVG；但若目标是其他图标主题，交给它们接管（九方守卫对称）
        const t = document.documentElement.dataset.theme;
        if (t !== 'night-sky' && t !== 'journal' && t !== 'strawberry' && t !== 'snow-country' && t !== 'sakura' && t !== 'animal' && t !== 'summer-rain' && t !== 'taro-choco') {
